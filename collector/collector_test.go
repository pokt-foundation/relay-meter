...
type fakeSource struct {
	...
	response    map[time.Time]map[types.PortalAppID]api.RelayCounts
	responseErr error

	todaysCounts           map[types.PortalAppID]api.RelayCounts
	todaysCountsPerOrigin  map[string]api.RelayCounts
	todaysLatency          map[types.PortalAppID][]api.Latency
	todaysMetricsCollected bool
	dailyMetricsCollected  bool
	todaysLatencyCollected bool
}

func (f *fakeSource) DailyCounts(from, to time.Time) (map[time.Time]map[types.PortalAppID]api.RelayCounts, error) {
...
func (f *fakeSource) TodaysCounts() (map[types.PortalAppID]api.RelayCounts, error) {
...
func (f *fakeSource) TodaysLatency() (map[types.PortalAppID][]api.Latency, error) {
...
func (f *fakeWriter) WriteDailyUsage(counts map[time.Time]map[types.PortalAppID]api.RelayCounts, countsOrigin map[string]api.RelayCounts) error {
	return nil
}

func (f *fakeWriter) WriteTodaysMetrics(counts map[types.PortalAppID]api.RelayCounts, countsOrigin map[string]api.RelayCounts, latencies map[types.PortalAppID][]api.Latency) error {
	f.todaysWrites++
	f.todaysLatencyWrites++
	return nil
}

func (f *fakeWriter) WriteTodaysUsage(ctx context.Context, tx *sql.Tx, counts map[types.PortalAppID]api.RelayCounts, countsOrigin map[string]api.RelayCounts) error {
	f.todaysWrites++
	return nil
}
